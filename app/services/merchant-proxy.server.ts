...
export async function authenticateMerchantProxy(request: Request) {
  ...
  if (!session || session.isOnline || session.shop !== shop)
    throw new Response("This store has not connected Refund.", { status: 404 });
...
## No Refund plugin or connector required
...
<p>Verify your purchase with Shopify, review the exact quote, and confirm before anything is submitted. No Refund account, plugin, or connector is required.</p>
<p><a href="${escapeHtml(discovery.browser.portalUrl)}" target="_top" rel="noreferrer">Continue securely with Refund</a></p>
