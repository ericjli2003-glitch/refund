...
export async function startReturnIntake(input: unknown) {
  ...
  return {
    ...
    nextStep:
      "Open continueUrl in the browser. If the browser supports background tabs, prefer one until customer interaction is needed. Ask the customer to complete Shopify verification themselves in a visible page and keep passwords and sign-in codes on Shopify's page. When the portal returns, use get_return_session, find_returnable_items, and quote_return, and present the results in the conversation without asking the customer to use the purchase form. Keep the portal loaded in the background if supported; do not close it or navigate it away while using its Site Tools. Gooper.io cannot control the host browser panel's visibility. Stop after showing the exact quote; do not submit unless the customer later gives explicit confirmation. If interrupted, reopen continueUrl and use get_return_session. Give support the correlationId, never credentials.",
  };
}
