...
- Because account recovery can route around MFA, the Google account's recovery
  phone and recovery address are reviewed at the same cadence as access (§7),
  and no sign-in method weaker than an authenticator app is enabled.
  `[CONFIRM: domain transfer lock at the registrar; no SMS sign-in method on
  the Google account; recovery phone and address current.]`
...
- Those vaults are unlocked by the Google account and the Apple ID, which
  ...
  every stored password. The Google account's MFA is confirmed above; the
  Apple ID carries two-factor authentication on the same basis.
  `[CONFIRM: two-factor authentication on the Apple ID.]`
