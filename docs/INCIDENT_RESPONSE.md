...
Records are kept in
[`ericjli2003-glitch/gooper-security`](https://github.com/ericjli2003-glitch/gooper-security),
a private repository separate from this one, so they survive an incident
affecting Render, carry their own timestamps and history, and can be produced
if Shopify or a regulator asks for them. One file per incident under
`incidents/`, from the `TEMPLATE.md` there, opened when the incident is
declared rather than written afterwards.

That repository also holds dated backup restore results (§5), access reviews
(§7) and post-incident reviews (§4.6).

Never record personal data belonging to an affected customer in an incident
record — reference the affected rows by identifier and count instead.
...
- The domain has registrar transfer lock and domain privacy enabled
  ...
  WHOIS does not list the operator's personal details.
- The Google account signs in with a passkey, Google prompt or an
  authenticator app. Text-message and voice sign-in are disabled (confirmed
  2026-09-20): an attacker can choose whichever method is weakest, and a
  phone number can be moved to another SIM. Backup codes are kept in Apple
  Passwords, not Google Password Manager, so being locked out of Google never
  also locks away the codes that recover it.
- Account recovery can route around MFA, so recovery options are reviewed at
  the same cadence as access. The Google account's recovery phone is current
  and its recovery email is a separate Google account (confirmed 2026-09-20).
  That recovery account can itself be used to recover the production one, so
  it needs MFA of its own.
  `[CONFIRM: two-step verification on the recovery Google account.]`
- Credentials are unique per service and generated by a password manager.
  Google Password Manager and Apple Passwords are in use (confirmed
  2026-09-20). Each production account's password is kept in one of the two,
  not both, so a rotation never leaves a stale copy behind.
- Those vaults are unlocked by the Google account and the Apple ID, which
  makes both of them production accounts in effect: whoever holds either holds
  every stored password. Both have two-factor authentication (confirmed
  2026-09-20).
...
| **SIM-swap exposure.** One mobile number is both the Google account's recovery phone and the Apple ID's trusted number. Anyone who moves it to their own SIM gains a recovery path into both. A carrier port-out PIN closes this. | Open — not yet confirmed |
