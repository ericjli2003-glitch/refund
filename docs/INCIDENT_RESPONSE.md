...
| Deputy | `[CONFIRM: name, or "none — sole operator"]` | Acts when the lead is unreachable |

**Internal reporting:** `[CONFIRM: email or channel for reporting a suspected incident]`
...
- **Shopify — within 24 hours of discovering an incident involving Shopify
  data,** including any protected customer data or merchant access tokens.
  `[CONFIRM: exact contractual window and channel per the Shopify API License
  and Terms of Use — verify before relying on the 24-hour figure here.]`
...
cause, and follow-up actions. Store outside the production database so it
survives the incident.

`[CONFIRM: where incident records are kept.]`

---

## 7. Access control

- Production access is limited to those who require it for their role.
  `[CONFIRM: who currently holds Render, GitHub, Shopify Partner, and Resend
  access.]`
- Multi-factor authentication is required on every account with production
  access. `[CONFIRM: MFA enforced on Render, GitHub, Shopify Partner, Resend.]`
