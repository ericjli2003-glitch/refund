...
function quote(number: string) {
  ...
  return { ...value, items: value.items.map(item => ({ ...item, title: `Item ${number}` })), orderName: `#${number}`,
    quoteToken: signQuote(value), expiresAt: new Date(expiresAt).toISOString(), paymentMethod: "Original", returnFees: { restocking: null, returnShipping: null }, returnShipping: "Store instructions", nextStep: "Stop at quote" };
}
try {
